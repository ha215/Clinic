...
class User extends Authenticatable implements HasMedia, MustVerifyEmail
{
    ...
    protected $fillable = [
        ...
        'user_type', 'civil_id', 'department_id', 'employee_id', 
        'pme_month', 'pme_year'
    ];
    ...
    public function encounterMedicalReports()
    {
        return $this->hasMany(EncounterMedicalReport::class, 'user_id', 'id');
    }

    // Method to find the latest (previous) test date
    public function previousTestDate()
    {
        $latestReport = $this->encounterMedicalReports()->orderBy('date', 'desc')->first();
        return $latestReport ? $latestReport->date : null;
    }

    // Method to find the upcoming test date (one year after the last test)
    public function upcomingTestDate()
    {
        $latestReport = $this->encounterMedicalReports()->orderBy('date', 'desc')->first();

         // Check if there's a latest report
        if ($latestReport) {
           // Convert the date string to a Carbon instance
            $testDate = Carbon::parse($latestReport->date);
        
           // Add the PME year and month to the test date
           $pmeYear = $this->pme_year ?? 0; // Default to 0 if not set
           $pmeMonth = $this->pme_month ?? 0; // Default to 0 if not set

           return $testDate->addYears($pmeYear)->addMonths($pmeMonth);
        }
